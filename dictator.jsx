...
class DictGame extends React.Component {

    constructor(){
        super(); 
        const dict_money = 100; 
        const offer_money = Math.floor(Math.random()*(dict_money - 1) + 1);
        this.state = {fileDownloadUrl: null, value:"", money: offer_money, dictMoney: dict_money};
        ...
        this.s3 = new AWS.S3({params: {Bucket:'dictdestinationbucket'}, region:"us-east-2", accessKeyId:'ACCESS-ID', 
        secretAccessKey:'SECRET-ACCESS-ID'})
...
divMoney(){
    ...
    this.setState({value:"", money:offer_money, dictMoney:dict_money});
}
    ...
    render () { 
        ...
        <body style={background}>
            ...
            <table style={b_style}>
                <tbody>
                    <tr>
                        <td>The Dictator has: {this.state.dictMoney}</td>
                        <td>The Dictator is offering: {this.state.money}</td>
                    </tr>
                </tbody> 
            </table>                
            </form>
            ...
            {this.state.value === "YES_STATE"?
                <div style={pa_styles}>
                    ...
                    <button onClick={() => {this.divMoney()}} disabled={!this.state.value}>Play Again?</button>
                </div>
                : null
            }
